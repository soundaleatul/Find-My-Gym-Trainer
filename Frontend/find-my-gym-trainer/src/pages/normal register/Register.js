  ...
  const inputstyle = {
    ...
    background: "transparent",
      ...
      const onRegister=(e)=>{
        ...
        e.preventDefault();
        if(firstname==''||lastname==''||password==''||password<password.length||contactno==''||email==''||startdate==''||enddate==''||pakageId==''||trainerId==''){
          window.alert('please enter all fields');
        }
        let customer={
          fName:firstname,lName:lastname,phNo:contactno,email:email,
          password:password,start_date:startdate,end_date:enddate,package_id:pakageId,trainer_id:trainerId,feeStatus:'unPaid'
        };
  ...
  return (
      ...
      <div className="container mt-3 mb-3 p-5" style={{"margin-left":"500px","width":"40%","border":"2px solid black","boxShadow":"1px 2px 3px"}}>
        <form>
          ...
          <div className="form-group row-md-4">
            <label htmlFor="pkgid">Select Package</label>
            {/* <input
              className="form-control"
              type="number"
             // value={pakageId}
            ...
            <select className="form-control "  style={inputstyle}
                  ...
                  {pkgdata.map((item)=>{
                                  ...
                                  return <option className="form-control"  value={item.id}>{item.description}</option>;
                  })}
                  </select>
          </div>
          <div className="form-group row-md-4">
            ...
            <select className="form-control "  style={inputstyle}
                  ...
                  {trainerdata.map((item)=>{
                                  ...
                                  return(<option className="form-control"  value={item.id}>{item.firstName} {item.lastName}</option>);
                  })}
                  </select>
          </div>
          <div className="form-group row-md-4">
          ...
          <div className="form-group row-md-4 m-2 text-center">
            <div className="col">
              <button
